...
Proposal.
1. **Root → the editor.** Done 2026-09-05: `swarmtyp.gwei` points at the release feed manifest (`b656fac5…a100`), so `https://swarmtyp.gwei.domains/` and `bzz://swarmtyp.gwei/` open the current app and every release is a feed update, no transaction. The gateway gives the app its own origin (`swarmtyp.gwei.domains`), which removes T14 for gateway users; Freedom already isolates per name. Verify first on a subdomain (`app.swarmtyp.gwei`) that the 12 MB load, the worker and the fonts work through the gateway, then re-point the root.
2. **swarmtyp's own pages** live under paths of the root collection. Done 2026-09-05 for the guide: `tools/guide/build-guide.mjs` turns `docs/user-guide.md` into `public/guide/index.html` on every dev and build, so `swarmtyp.gwei/guide/` is the guide and the topbar links to it. It is flowing HTML, not a paged Typst site, on purpose: a paged site is an A4 page scaled to the screen, which on a phone means either tiny text or sideways scrolling, so the paged form (D-24) suits documents meant as pages and the guide is meant to be read on anything. When Typst's HTML export is usable the guide can move to Typst; until then Markdown stays its source. `swarmtyp.gwei/demo/` for the S11 sample is still to do. Not as subdomains: S11 found that `<sub>.<name>.gwei.domains` has no TLS certificate on either public gateway (wildcards cover one label), so subdomains resolve in Freedom only. `app.swarmtyp.gwei` exists from the test and stays as a Freedom-only alias.
...
4. **Reads without a node.** When the configured Bee URL does not answer, the app reads fonts, packages and blobs from `download.gateway.ethswarm.org` (CORS open; whole bodies, since its cache ignores Range offsets, S11); writes still need a node and a stamp, shown in a banner. Built 2026-09-05 (`PUBLIC_READ_GATEWAY` in `src/swarm/bee.ts`): with no node the starter compiles in 4 s from the gateway. This is the "read gateway" of D-22 without running one, and it is what makes the root name usable from a browser, because a page on a public origin cannot reach `127.0.0.1` anyway.
5. **A background text inside the editor**: done 2026-09-05 as the starting document itself (`src/app/starter.typ`: what swarmtyp is, two CeTZ diagrams, formulas) plus a Guide link in the topbar. A separate About panel is not needed while the starter carries the text; a local document that still equals an older starter is replaced on upgrade, an edited one is kept.

Bee and Swarm Desktop cannot resolve `.gwei` yet (2026-09-05): Bee's ENS client treats the configured contract as an ENS registry (`owner`, `resolver`, then `contenthash`), while GNS exposes the ENS resolver profile (`contenthash(bytes32 node)`, `supportsInterface(0xbc1c58d1)`) on the name contract itself with no registry. Filed as ethersphere/bee#5600 with a proposed two-line change in `pkg/resolver/client/ens/ens.go` (probe the profile, then `goens.NewResolverAt(...).Contenthash()`). Until it lands, node users open the raw manifest URL, which the guide lists; the name is for gateway and Freedom users.
