...
2. **swarmtyp's own pages** (the user guide written in Typst and published with the paged-site path, so the guide is also the first real published document; the S11 sample as the demo) live under paths of the root collection, `swarmtyp.gwei/guide/` and `swarmtyp.gwei/demo/`, added to `dist/` by the release tooling. Not as subdomains: S11 found that `<sub>.<name>.gwei.domains` has no TLS certificate on either public gateway (wildcards cover one label), so subdomains resolve in Freedom only. `app.swarmtyp.gwei` exists from the test and stays as a Freedom-only alias.
