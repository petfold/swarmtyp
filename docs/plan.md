...
Done when: every spike has a recorded result and D-03, D-04, D-08 have moved from PROPOSED/OPEN to DECIDED.
...
- Compile worker with typst.ts; preview pane; diagnostics in the gutter.
...
Comments and suggestions as a second CRDT layer; templates from Swarm; a read-only "viewer" build for published documents; a local-first mode that queues writes while a Bee node is unreachable.
