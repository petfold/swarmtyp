...
Result, part 1 continued: the editor through a name (2026-09-05, owner minted `app.swarmtyp.gwei` and set its website to the release feed manifest; the "Set Website" step was done by Claude in the owner's browser, the MetaMask confirmation by the owner).

| Where | Result for the editor by name |
|---|---|
| Freedom Browser, `bzz://app.swarmtyp.gwei/` | loads and compiles the starter (335 s cold through Ant, `compiled in 923 ms`, one page); origin `bzz://app.swarmtyp.gwei` |
| `https://app.swarmtyp.gwei.domains/` | **TLS handshake failure**: the gateway's certificate covers `gwei.domains` and `*.gwei.domains` only, so a subdomain of a name (two labels deep) has no certificate; `gwei.site` fails the same way (its certificate names its Deno cluster). Plain `http://` answers 200 |
| `http://app.swarmtyp.gwei.domains/` (Chromium, test only) | the app loads through the gateway: 16.5 MB in 30 s, `compiler ready` at 29 s; the first build then reported `compiled in 69 ms, 1 error` with no page, because every read went to `http://127.0.0.1:1633` and a page served from a public origin cannot reach a private address (Chromium's private-network access rules; also nobody on a gateway has a node there) |
| Public read gateway, `download.gateway.ethswarm.org/bytes/<ref>` | answers `Range` with 206 and `Access-Control-Allow-Origin: *`, **but returns bytes 0–1048575 for every offset** (a cache in front ignores the Range offset) and hides `Content-Range` cross-origin, so the piece walk never ended: 415 requests for one 1.4 MB font. Fixed in `src/compile/ranged.ts`: whole-body GET for that host, and any walker that receives a repeated piece falls back to a whole-body GET. Second finding the same evening: the cache also answered a request *without* a Range header with the cached 206 (first piece), so whole-body requests to that host go under their own cache key (`?whole=1`, which Bee ignores) and retry once under a fresh key if a 206 still comes back |
| No node at all (`beeUrl` unreachable, preview build) | with the read fallback (D-25 item 4, `PUBLIC_READ_GATEWAY`): banner shown, fonts and the package read from the gateway, `compiled in 2546 ms`, one page, 5 gateway requests, 4 s from load |
| Release `e056bdd0…4950` (read fallback, loader fix) through the gateway, `http://app.swarmtyp.gwei.domains/` | loads and compiles with a page: 18.9 MB, `compiler ready` at 20 s, `compiled in 3915 ms` at 29 s, fonts from the public read gateway, the only failed request the probe to `127.0.0.1:1633`. The root name is one label deep, so over `https://` it has a certificate |

| Root `swarmtyp.gwei` → release feed manifest (owner's transaction 2026-09-05 18:20 UTC, form filled by Claude) | gateway cache turned over after 4 min; `https://swarmtyp.gwei.domains/` then loaded the editor over TLS with its own origin: 18.5 MB, `compiled in 2982 ms` at 15 s, one page, fonts from the public read gateway |

Consequences: (a) **subdomains of a `.gwei` name are Freedom-only** on the web today, so swarmtyp's own pages (guide, demo) go under paths of the root collection or under first-level names, not `guide.swarmtyp.gwei` (D-25 revised); (b) the root can carry the editor once the release with the read fallback is verified through the gateway; (c) the public gateway's Range behaviour is worth reporting to the Swarm Foundation.
